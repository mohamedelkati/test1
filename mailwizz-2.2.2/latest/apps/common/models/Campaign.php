 ...
 * @property string|null $send_between_end
...
class Campaign extends ActiveRecord
{
    ...
    public function rules()
    {
        $rules = [
            ...
            // since 2.2.0
            ['send_between_start, send_between_end', 'date', 'format' => 'hh:mm:ss', 'on' => 'step-confirm'],
            ['send_between_start, send_between_end', 'in', 'range' => array_keys($this->getSendBetweenHoursList()), 'on' => 'step-confirm'],
            ['send_between_start', 'compare', 'allowEmpty' => true, 'compareAttribute' => 'send_between_end', 'operator' => '<', 'message' => $this->t('"{firstAttribute}" must be less than "{secondAttribute}"', [
                '{firstAttribute}'  => $this->getAttributeLabel('send_between_start'),
                '{secondAttribute}' => $this->getAttributeLabel('send_between_end'),
            ]), 'on' => 'step-confirm'],
            ['send_between_end', 'compare', 'allowEmpty' => true, 'compareAttribute' => 'send_between_start', 'operator' => '>', 'message' => $this->t('"{firstAttribute}" must be greater than "{secondAttribute}"', [
                '{firstAttribute}'  => $this->getAttributeLabel('send_between_end'),
                '{secondAttribute}' => $this->getAttributeLabel('send_between_start'),
            ]), 'on' => 'step-confirm'],
    ...
    public function attributeLabels()
    {
        $labels = [
            ...
            'send_between_end'      => $this->t('Daily sending window - End hour'),
    ...
    public function attributeHelpTexts()
    {
        $texts = [
            ...
            'send_between_start'    => $this->t('Send the campaign only between given hours'),
            'send_between_end'      => $this->t('Send the campaign only between given hours'),
    ...
    public function getDailySendingWindowDisplay(): string
    {
        ...
        if (empty($this->send_between_start) || empty($this->send_between_end)) {
            return $text;
        }

        try {
            $text = $this->t('From {start} to {end}', [
                '{start}' => $this->dateTimeFormatter->formatLocalizedTime(sprintf('%s %s', MW_DATE_NOW, $this->send_between_start)),
                '{end}' => $this->dateTimeFormatter->formatLocalizedTime(sprintf('%s %s', MW_DATE_NOW, $this->send_between_end)),
            ]);
        } catch (Exception $e) {
            Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
        }
    ...
    protected function beforeSave()
    {
        ...
        if (empty($this->send_between_end)) {
            $this->send_between_end = null;
        }
